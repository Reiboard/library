...
def creation(*args):
    ...
    for a in mirrorGroup:
        cmds.mirrorJoint( a, searchReplace=('R_', 'L_'))
    ...
    for a in IKgroup:
        cmds.mirrorJoint( a, searchReplace=('SK_', 'IK_'))
        cmds.mirrorJoint( a, searchReplace=('SK_', 'FK_'))
    ...
    #reduce_circle
    groupCircle = cmds.ls('*_ctl')
    for i in groupCircle:
        cmds.select(i + '.cv[0:7]')
        cmds.scale(0.5, 0.5, 0.5)
    
    circleFinger = cmds.ls('*Finger*_ctl') + cmds.ls('*cup*_ctl')
    
    for i in circleFinger:
        cmds.select(i + '.cv[0:7]')
        cmds.scale(0.5, 0.5, 0.5)
    ...
    for i in wristConstraint:
        for z in LR:
            ...
            cmds.setAttr('orige_SK_' + z + i + '_grp_parentConstraint1.FK_'+z+'wrist_ctlW0', 1)
            cmds.setAttr('orige_SK_' + z + i + '_grp_parentConstraint1.controler_IK_handle_arm_'+z+'ctlW1', 0)
            cmds.setDrivenKeyframe('orige_SK_' + z + i + '_grp_parentConstraint1.FK_'+z+'wrist_ctlW0', currentDriver = 'FK_'+z+'shoulder_ctl.IK_FK')
            cmds.setDrivenKeyframe('orige_SK_' + z + i + '_grp_parentConstraint1.controler_IK_handle_arm_'+z+'ctlW1', currentDriver = 'FK_'+z+'shoulder_ctl.IK_FK')
    #arm_setKey_IK        
    for i in wristConstraint:
        for z in LR:
            cmds.setAttr('FK_'+z+'shoulder_ctl.IK_FK', 0)
            cmds.setAttr('orige_SK_' + z + i + '_grp_parentConstraint1.FK_'+z+'wrist_ctlW0', 0)
            cmds.setAttr('orige_SK_' + z + i + '_grp_parentConstraint1.controler_IK_handle_arm_'+z+'ctlW1', 1)
            cmds.setDrivenKeyframe('orige_SK_' + z + i + '_grp_parentConstraint1.FK_'+z+'wrist_ctlW0', currentDriver = 'FK_'+z+'shoulder_ctl.IK_FK')
            cmds.setDrivenKeyframe('orige_SK_' + z + i + '_grp_parentConstraint1.controler_IK_handle_arm_'+z+'ctlW1', currentDriver = 'FK_'+z+'shoulder_ctl.IK_FK')
    ...
    for i in ankleConstraint:
        for z in LR:
            ...
            cmds.setAttr('orige_SK_' + z + i + '_grp_parentConstraint1.FK_'+z+'ankle_ctlW0', 1)
            cmds.setAttr('orige_SK_' + z + i + '_grp_parentConstraint1.controler_IK_handle_leg_'+z+'ctlW1', 0)
            cmds.setDrivenKeyframe('orige_SK_' + z + i + '_grp_parentConstraint1.FK_'+z+'ankle_ctlW0', currentDriver = 'FK_'+z+'leg_ctl.IK_FK')
            cmds.setDrivenKeyframe('orige_SK_' + z + i + '_grp_parentConstraint1.controler_IK_handle_leg_'+z+'ctlW1', currentDriver = 'FK_'+z+'leg_ctl.IK_FK')
    #leg_setKey_IK
    for i in ankleConstraint:
        for z in LR:
            cmds.setAttr('FK_'+z+'leg_ctl.IK_FK', 0)
            cmds.setAttr('orige_SK_' + z + i + '_grp_parentConstraint1.FK_'+z+'ankle_ctlW0', 0)
            cmds.setAttr('orige_SK_' + z + i + '_grp_parentConstraint1.controler_IK_handle_leg_'+z+'ctlW1', 1)
            cmds.setDrivenKeyframe('orige_SK_' + z + i + '_grp_parentConstraint1.FK_'+z+'ankle_ctlW0', currentDriver = 'FK_'+z+'leg_ctl.IK_FK')
            cmds.setDrivenKeyframe('orige_SK_' + z + i + '_grp_parentConstraint1.controler_IK_handle_leg_'+z+'ctlW1', currentDriver = 'FK_'+z+'leg_ctl.IK_FK')
    ...
    #transfert_parent
    for i in LR:
        cmds.parent('parentConstraint_SK_' + i +'heel_grp', 'IK_' + i +'ankle_jnt')
        cmds.parent('parentConstraint_SK_' + i +'toes_grp', 'IK_' + i +'ankle_jnt')
        cmds.parent('parentConstraint_SK_' + i +'cup_grp', 'FK_' + i +'wrist_jnt')
        cmds.parent('parentConstraint_SK_' + i +'middleFinger1_grp', 'FK_' + i +'wrist_jnt')
        cmds.parent('parentConstraint_SK_' + i +'indexFinger1_grp', 'FK_' + i +'wrist_jnt')
        cmds.parent('parentConstraint_SK_' + i +'thumbFinger1_grp', 'FK_' + i +'wrist_jnt')
    
    
    #hide_SK_joint
    SK_joint_hide = ['scapula', 'leg']
    
    for i in SK_joint_hide:
        cmds.select('SK_*_' + i + '_jnt')
        u = cmds.ls(sl = True)
        print(u)
        for z in u:
            cmds.setAttr(z + '.visibility', 0)
...
cmds.button( label='first_phase', command=createSkeleton)
cmds.button( label='second_phase', command=creation)
cmds.showWindow()
